...
def _save_annotation(annotation, filename):
  ...
  """
  #annotation.astype(dtype=np.unit8)
  print (annotation.ndim)
  annotation[annotation>0]=255
  annotation[annotation<=0]=0
  #if annotation[i,j,k]
  pil_image = Image.fromarray(annotation.astype(dtype=np.uint8))
  #pil_image = Image.fromarray(annotation.astype(dtype=np.unit8))
...
def main(unused_argv):
  ...
  ops = ['TPDEB','TPPFG','TPWR0','TTFBS','TTSAD','TTSPS','TCFBA']
  for op in ops:

    output_dir = FLAGS.output_dir+op+'/'
    if not tf.gfile.IsDirectory(output_dir):
      tf.gfile.MakeDirs(output_dir)
    original_gt_folder = FLAGS.original_gt_folder+op+'/'
    annotations = glob.glob(os.path.join(original_gt_folder,
